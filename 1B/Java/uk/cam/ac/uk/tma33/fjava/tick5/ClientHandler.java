...
public class ClientHandler {
    ...
    * @param d	   	 The database instance we want to use for this chat server 
    * @throws SQLException		If the database connection fails
    */

   public ClientHandler(Socket s, MultiQueue<Message> q, Database d) throws SQLException{
      socket = s;
      multiQueue = q;
   ...
   /**
    * 		Gets the most recent 10 messages from the chat and sends them to the client
    * 				Method only called when the client connects for the first time
    * 
    * @throws SQLException		If connection goes wrong
    */
   private void getLast10Msgs() throws SQLException{
	   ArrayList<RelayMessage> list = (ArrayList) database.getRecent();
      int lng = list.size();
      for (int i = lng - 1; i >= 0; i--) {
         RelayMessage msg = list.get(i);
